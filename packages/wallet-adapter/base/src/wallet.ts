...
import { isBBAChain, type BBAChain } from '@bbachain/wallet-standard-chains';
import {
...
export class BBAChainWalletAdapterWallet implements Wallet {
    ...
    constructor(adapter: Adapter, chain: BBAChain, endpoint?: string) {
    ...
    #signTransaction: BBAChainSignTransactionMethod = async (...inputs) => {
        ...
        if (inputs.length === 1) {
            const input = inputs[0]!;
            if (input.account !== this.#account) throw new Error('invalid account');
            if (input.chain && !isBBAChain(input.chain)) throw new Error('invalid chain');
            const transaction = this.#deserializeTransaction(input.transaction);
